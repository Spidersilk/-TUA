...
- (void)request{
   ...
   [RequestAPI requestURL:@"/login/getKey" withParameters:prarmeter andHeader:nil byMethod:kGet andSerializer:kForm success:^(id responseObject) {
        ...
        if ([responseObject[@"resultFlag"] integerValue] == 8001) {
           ...
           //对内容进行MD5加密
            NSString *md5Str = [_pswTextField.text getMD5_32BitString];
...
- (void)signUpWithEncryptPwd:(NSString *)encryptPwd {
    NSString *str = [Utilities uniqueVendor];
    [RequestAPI requestURL:@"/register" withParameters:@{@"userTel" : _usernameTextField.text, @"userPwd" : encryptPwd,@"nickname" : _nicknameTextField.text,@"nums" : _codeTextField.text, @"deviceType" : @7001, @"deviceId" : str} andHeader:nil byMethod:kPost andSerializer:kJson success:^(id responseObject) {
        [_avi stopAnimating];
        NSLog(@"responseObject = %@",responseObject);
        if ([responseObject[@"resultFlag"] integerValue] == 8001) {
//            NSDictionary *result = responseObject[@"result"];
...
- (IBAction)signUpAction:(UIButton *)sender forEvent:(UIEvent *)event {
    ...
    if(_pswTextField.text.length == 0){
        [Utilities popUpAlertViewWithMsg:@"请输入您的密码" andTitle:nil onView:self];
        return;
    }
    if(_pswTextField.text.length < 6 || _pswTextField.text.length > 18){
